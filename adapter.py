...
class WebchatAdapter(BasePlatformAdapter):
    ...
    def _setup_routes(self) -> None:
        ...
        # Per-message actions
        self._app.router.add_post("/api/branch", self._handle_branch)
        self._app.router.add_post("/api/undo", self._handle_undo)
        self._app.router.add_post("/api/command-panel", self._handle_command_panel)
    ...
    async def _handle_branch(self, request: web.Request) -> web.Response:
        """POST /api/branch — branch the current session from this point."""
        payload = self._require_auth(request)
        if not payload:
            return self._json_response({"error": "Unauthorised"}, status=401)
        try:
        ...
        session_id = str(data.get("session_id") or "").strip()
        if not session_id:
            return self._json_response({"error": "session_id required"}, status=400)
        ...
        await self.handle_message(event)
        _db_add_message(session_id, "user", "/branch")
        _db_add_message(session_id, "assistant", "Creating branch...")
        return self._json_response({"status": "ok", "message": "Branch created. Reload sessions to see it."})

    async def _handle_undo(self, request: web.Request) -> web.Response:
        """POST /api/undo — undo to a specific message."""
        payload = self._require_auth(request)
        if not payload:
            return self._json_response({"error": "Unauthorised"}, status=401)
        try:
        ...
        if not session_id or message_index is None:
            return self._json_response({"error": "session_id and message_index required"}, status=400)
        ...
        # Send /undo command to gateway — it only removes the last exchange.
        # For "undo to this point" we send it once per exchange needed.
        # Calculate: how many user messages after message_index?
        all_messages = _db_get_messages(session_id)
        msgs_after = len(all_messages) - message_index - 1
        undo_count = max(1, (msgs_after + 1) // 2)  # Each /undo removes 1 user + 1 assistant

        last_result = ""
        for _ in range(min(undo_count, 5)):  # Cap at 5 to prevent abuse
            event = MessageEvent(
                text="/undo",
                message_type=MessageType.TEXT,
                source=source,
        ...
        _db_add_message(session_id, "system", f"Undid to message {message_index}")
        return self._json_response({"status": "ok", "message": f"Undid {undo_count} exchange(s)"})

    # ── Command panel interactions ──────────────────────────────────────

    async def _handle_command_panel(self, request: web.Request) -> web.Response:
        """POST /api/command-panel — get interaction panel data for a slash command.

        Body: { "command": "/reasoning", "session_id": "..." }
        Returns interaction panel data that the frontend renders as an inline panel.
        """
        payload = self._require_auth(request)
        if not payload:
            return self._json_response({"error": "Unauthorised"}, status=401)
        ...
        if command == "/reasoning":
            return await self._get_reasoning_panel(session_id)
        elif command == "/personality":
            return await self._get_personality_panel(session_id)
        elif command == "/goal":
            return await self._get_goal_panel(session_id)
        elif command == "/background":
            return await self._get_background_panel(session_id)
        elif command == "/usage":
            return await self._get_usage_panel(session_id)
        else:
            return self._json_response({"error": f"Unknown command panel: {command}"}, status=400)

    async def _get_reasoning_panel(self, session_id: str) -> web.Response:
        """Return a reasoning effort selection panel."""
        controls = [
        ...
        return self._json_response({
            ...
            "panel": {
                ...
                "content": "**Reasoning** controls how much the model \"thinks\" before responding.\n\nSelect a reasoning level or toggle display.",
                "controls": controls,
                "secondary_controls": show_hide,
            }
        })

    async def _get_personality_panel(self, session_id: str) -> web.Response:
        """Return a personality selection panel."""
        controls = [
        ...
        return self._json_response({
            ...
            "panel": {
                ...
                "content": "Choose a personality for Hermes in this session. Each preset changes the tone and style of responses.",
                "controls": controls,
            }
        })

    async def _get_goal_panel(self, session_id: str) -> web.Response:
        """Return a goal management panel."""
        controls = [
        ...
        return self._json_response({
            ...
            "panel": {
                "title": "Standing goal",
                "content": "A **standing goal** is something Hermes works on across multiple turns.\n\n`Set goal`: Provide a goal description\n`Status`: Check current goal\n`Clear`: Remove the current goal",
                "controls": controls,
            }
        })

    async def _get_background_panel(self, session_id: str) -> web.Response:
        """Return a background task panel."""
        return self._json_response({
            "status": "ok",
            "panel": {
                "title": "Background task",
                "content": "Run a prompt in the background. The task continues even while you start a new conversation.\n\nType a prompt below and click **Run in background**:",
                "controls": [],
                "has_text_input": True,
                "input_placeholder": "e.g. Research the latest AI papers on reasoning...",
            }
        })

    async def _get_usage_panel(self, session_id: str) -> web.Response:
        """Return a usage info panel."""
        return self._json_response({
            "status": "ok",
            "panel": {
                "title": "Usage & token stats",
                "content": "Session usage information appears below after you request it.\n\nClick **Show usage** to retrieve current token counts and rate limits.",
                "controls": [
